...
{
    [Export]
    public class FastTicketListViewModel : ObservableObject
    {
        ...
        public FastTicketListViewModel(ITicketService ticketService, ITicketServiceBase ticketServiceBase,
            IApplicationState applicationState, IUserService userService)
        {
            ...
            _tickets = new List<FastTicketButtonViewModel>();
        ...
        private List<FastTicketButtonViewModel> _tickets;
        public List<FastTicketButtonViewModel> Tickets => _tickets;

        public IEntityClass SelectedEntity { get; set; }
        public string ListName => SelectedEntity != null ? SelectedEntity.Name : "";
        ...
        public string TotalRemainingAmountLabel => _tickets != null ? Tickets.Sum(x => x.RemainingAmount).ToString(LocalSettings.ReportCurrencyFormat) : "";
        public int RowCount => _tickets.Count() > 8 ? _tickets.Count() : 8;
        public int SelectedItemsCount => _tickets.Count(x => x.IsSelected);
        ...
        public void UpdateListByEntity(Entity entity)
        {
            if (entity != null)
            {
                ...
                _tickets = _ticketServiceBase.GetOpenTickets(entity.Id).Select(x => new FastTicketButtonViewModel(x, entity)).ToList();
                Refresh();
            }
        }
        ...
        public void UpdateListByTicketTagGroup(TicketTagGroup tagGroup)
        {
            ...
            _tickets = _ticketServiceBase.GetOpenTickets(x => !x.IsClosed && x.TicketTags.Contains(tagValue)).Select(x => new FastTicketButtonViewModel(x, null)).ToList();
            Refresh();
        }
        ...
        public void UpdateListByTicketState(TicketStateData value)
        {
            ...
            _tickets = _ticketServiceBase.GetOpenTickets(x => x.TicketStates.Contains(stateValue)).Select(x => new FastTicketButtonViewModel(x, null)).ToList();
            Refresh();
        }
