...
{
    [Export]
    public class FastOrderTagGroupEditorViewModel : ObservableObject
    {
        private readonly IApplicationState _applicationState;
        private readonly ICacheService _cacheService;
        private readonly FastAutomationCommandSelectorViewModel _automationCommandSelectorViewModel;

        [ImportingConstructor]
        public FastOrderTagGroupEditorViewModel(
            IUserService userService,
            IApplicationState applicationState,
            ICacheService cacheService,
            FastAutomationCommandSelectorViewModel automationCommandSelectorViewModel)
        {
            ...
            GroupedOrderTagSelectedCommand =
                new DelegateCommand<FastGroupedOrderTagButtonViewModel>(OnGroupedOrderTagSelected);

            OrderTagSelectedCommand =
                new DelegateCommand<FastOrderTagButtonViewModel>(OnOrderTagSelected);

            FreeTagSelectedCommand =
                new DelegateCommand<FastOrderTagGroupViewModel>(OnFreeTagSelected);

            SelectedItemPortions = new ObservableCollection<MenuItemPortion>();
            OrderTagGroups = new ObservableCollection<FastOrderTagGroupViewModel>();
            GroupedOrderTagGroups = new ObservableCollection<FastGroupedOrderTagViewModel>();
        }
        ...
        public string RemoveModeButtonColor
        {
            get { return RemoveMode ? "Black" : "Gainsboro"; }
        }

        public FastAutomationCommandSelectorViewModel AutomationCommandSelectorViewModel
        {
            get { return _automationCommandSelectorViewModel; }
        }

        public ICaptionCommand CloseCommand { get; set; }
        public CaptionCommand<string> ToggleRemoveModeCommand { get; set; }
        public DelegateCommand<FastOrderTagGroupViewModel> FreeTagSelectedCommand { get; set; }
        ...
        public DelegateCommand<FastOrderTagButtonViewModel> OrderTagSelectedCommand { get; set; }
        public DelegateCommand<FastGroupedOrderTagButtonViewModel> GroupedOrderTagSelectedCommand { get; set; }
        public ObservableCollection<FastOrderTagGroupViewModel> OrderTagGroups { get; set; }
        public ObservableCollection<FastGroupedOrderTagViewModel> GroupedOrderTagGroups { get; set; }
        ...
        {
            // FastPay tarafında kapatırken FastPay ekranına dön
            EventServiceFactory.EventService.PublishEvent(
                CurrentOperationRequest != null
        ...
        private void OnOrderTagSelected(FastOrderTagButtonViewModel orderTag)
        {
            var mig = OrderTagGroups
                .First(propertyGroup => propertyGroup.OrderTags.Contains(orderTag))
                .Model;
            ...
            orderTagData.PublishEvent(
                RemoveMode ? EventTopicNames.OrderTagRemoved : EventTopicNames.OrderTagSelected,
                true);

            RemoveMode = false;
            OrderTagGroups
                .Where(x => x.OrderTags.Contains(orderTag))
                .ToList()
                .ForEach(x => x.Refresh());
        }

        private void OnGroupedOrderTagSelected(FastGroupedOrderTagButtonViewModel obj)
        {
        ...
        private void OnFreeTagSelected(FastOrderTagGroupViewModel obj)
        {
            if (string.IsNullOrEmpty(obj.FreeTagName) ||
                string.IsNullOrEmpty(obj.FreeTagName.Trim())) return;
            ...
            var orderTagData = new OrderTagData
            {
                ...
                {
                    Name = obj.FreeTagName,
                    Price = obj.FreeTagPrice
                },
                Ticket = SelectedTicket
            };
            ...
            OrderTagGroups
                .Where(x => x.OrderTags.Any(y => y.Name == obj.FreeTagName))
                .ToList()
                .ForEach(x => x.Refresh());
        }
        ...
        public bool ShouldDisplay(Ticket value, IEnumerable<Order> selectedOrders)
        {
            ...
            if (SelectedTicket != null && SelectedOrder != null)
            {
                ...
                var orderTagGroups = _applicationState
                    .GetOrderTagGroups(SelectedOrder.MenuItemId)
                    .Where(x => !x.Hidden)
                    .ToList();
                ...
                OrderTagGroups.AddRange(
                    orderTagGroups
                        ...
                        .Select(x => new FastOrderTagGroupViewModel(x, so)));
                ...
                if (SelectedOrder != null)
                {
                    GroupedOrderTagGroups.AddRange(
                        orderTagGroups
                            ...
                            .Select(x => new FastGroupedOrderTagViewModel(SelectedOrder, x)));
                }
            }

            return SelectedItemPortions.Count > 1
                   || OrderTagGroups.Count > 0
                   || GroupedOrderTagGroups.Count > 0;
        }
    }
}
