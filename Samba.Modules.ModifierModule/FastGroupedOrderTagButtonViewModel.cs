...
{
    public class FastGroupedOrderTagButtonViewModel : ObservableObject
    {
        public FastGroupedOrderTagButtonViewModel(Order selectedItem, OrderTagGroup orderTagGroup)
        {
        ...
        public void UpdateNextTag(OrderTag current)
        {
            ...
            if (CurrentTag != null)
            {
                var nProp = OrderTagGroup.OrderTags
                    .SkipWhile(x => x.Name != CurrentTag.Name)
                    .Skip(1)
                    .FirstOrDefault();
        ...
        private OrderTag GetCurrentTag(OrderTag current)
        {
            ...
            var selected = _selectedItem
                .GetOrderTagValues()
                .FirstOrDefault(x => x.OrderTagGroupId == OrderTagGroup.Id);
        ...
        public int FontSize
        {
            get { return OrderTagGroup.FontSize > 0 ? OrderTagGroup.FontSize : 16; }
        }

        public string Color
        {
            get { return OrderTagGroup.ButtonColor ?? "Gainsboro"; }
        }
    }
}
