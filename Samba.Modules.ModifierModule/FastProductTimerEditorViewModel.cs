...
{
    [Export]
    public class FastProductTimerEditorViewModel : ObservableObject
    {
        [ImportingConstructor]
        public FastProductTimerEditorViewModel()
        {
        ...
        {
            get { return IsActive ? DateTime.Now : SelectedOrder.ProductTimerValue.End; }
        }

        public bool IsActive
        {
            get { return SelectedOrder.ProductTimerValue.IsActive; }
        }

        public string Start
        {
            get { return SelectedOrder.ProductTimerValue.Start.ToString(); }
        }

        public string End
        {
            get { return IsActive ? Resources.Active : SelectedOrder.ProductTimerValue.End.ToString(); }
        }
        ...
        public string Duration
        {
            get
            {
                return TimeSpan.FromTicks(EndDate.Ticks - StartDate.Ticks).ToLongDuration();
            }
        }

        public string Price
        {
            get { return GetPriceDescription(); }
        }

        public string Value
        {
            get { return GetValueDescription(); }
        }

        private string GetPriceDescription()
        {
            var mi = SelectedOrder.ProductTimerValue;
            return string.Format("{0:#} {1} {2}: {3:N}",
                                 mi.PriceDuration,
                                 GetTimeDescription(mi.PriceType),
                                 Resources.Price,
                                 SelectedOrder.Price);
        }
        ...
        {
            if (selectedOrders == null || selectedOrders.Count() > 1 || !selectedOrders.Any())
                return false;
        ...
        private void OnCloseCommandExecuted(string obj)
        {
            ...
            // FastPay tarafında kapatınca FastPay ekranına dön
